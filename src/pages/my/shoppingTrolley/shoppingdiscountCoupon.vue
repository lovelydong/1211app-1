        ...
        <div class="mid">
          <ul>
            <li v-for="(c,index) in coupon">
              ...
              <f7-link @click="fn(c.id)" :href="state='立即使用'?' ':''" id="gg" :class="(c.state==1&&c.over_time>nowtime)?'':'gg' "  :html="(c.state==1&&c.over_time)>nowtime?state='立即使用':(c.state==2?state='已使用':(c.state==3?state='已失效':state='已过期'))">{{state}}</f7-link>
            </li>
           <!-- <li>
              <span>券</span>
              <p>限教师资格品类直播课程使用</p>
...
export default {
  ...
  methods: {
    fn(id){
      Global.usershoppingID = id;
      this.$f7router.back("/indent?order_number=" + this.$f7route.query.order_number,{force :true});
    }
  },
  created:function()
  {
  	...
  	this.$http
        .get(this.url + "/sxcouponsend/getCoupon" + this.ids, {
