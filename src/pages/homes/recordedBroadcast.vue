<template>
  <f7-page class="recordedBroadcast"  ptr @ptr:refresh="onRefresh" infinite @infinite="onInfinite" >
        <f7-navbar>
          ...
          <div class="TopsZ" v-if="showTop" >
            ...
            <transition
              ...
              <div class="lx" v-if="shows == 1?true:false">
                  <ul>
                    <li v-for="item in gitCon.exam_type" class="col-33" :class="{active: type.exam_type == item.name_value ?true:false}" @click="type.exam_type = item.name_value;exam_typeFn(item.name_value)"> {{item.name}} <i v-if="type.exam_type == item.name_value ?true:false" class="iconfont icon-selected-copy"></i></li>
                  </ul>
              </div>
            </transition>
             <transition
            ...
            <transition
                ...
                >
                  <div class="screen-but" v-if="shows == 4?true:false">
                    ...
                    <div class="off">
                      <div class="row">
                          <div class="col-50" @click="type.grade =null;type.subject =null;type.course_type =null;type.province =null;">重置</div>
...
.recordedBroadcast {
  ...
  .toolbarlog3 {
    background: url(../../assets/jingling.png) no-repeat -6px -87px;
    background-color: transparent !important;
  }
  .toolbarlog4 {
